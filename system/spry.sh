...
# TODO: This might be better as a Spry System function
function _spry_refresh() {
    ...
    local result=$(_spry_generate_hash 2>&1)

    # If somethin is different in the code OR you're trying to load under a different mode
    if [[ $result != $SPRY_CLI_VERISON_HASH ]] || [[ ${1} != ${SPRY_MODE} ]]; then
        ...
        format info "This shell has already sourced the current" partial
        format spry partial
        format info " files"
...
function _spry_generate_hash() {
    # This creates a hash of the directory (except for sepcified sub-folders) and contents for version checking

    local counters_dir='Users/sethcottam/spry_cli/system/faux_db/counters'

    # tar cfP - ${SPRY_CLI_ROOT} | md5  # Original that doesn't exclude
    tar cP --exclude=${counters_dir} -c ${SPRY_CLI_ROOT} | md5
}
...
function _spry_pull() {
    ...
    #   - We'll want to redirect it to pull personal spryts from a personal repo, but I'm not sure how to do that yet.
