...
local AUTOLOADER_ARG="development"  # If you need to manually change this after an install run: vim $HOME/.zshrc
...
check_shell() {
    ...
    if [[ ${result} =~ "/bin/zsh"  ]]; then
        ...
        error "Bash is not currently a fully compatible shell"
        installation_failure
    else
...
source_files() {
    ...
    if [[ -z ${result} ]]; then
        ...
        echo "export SPRY_CLI_CUSTOM_ROOT_DIRECTORY=\"/Users/sethcottam/spry_cli_custom\""
