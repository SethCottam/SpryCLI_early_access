...
local total_todos=$(stats todo ${SPRY_CLI_ROOT_DIRECTORY}/spryts)
local total_functions=$(stats functions ${SPRY_CLI_ROOT_DIRECTORY}/spryts)
local total_comments=$(stats comments ${SPRY_CLI_ROOT_DIRECTORY}/spryts)
local total_shell_spryts=$(stats shell_spryts ${SPRY_CLI_ROOT_DIRECTORY}/spryts)
local total_python_spryts=$(stats python_spryts ${SPRY_CLI_ROOT_DIRECTORY}/spryts)
local total_families=$(stats families)
# local total_children="WIP"
local total_tests="WIP"

# Get the combined totals
local total_aliases=$(( $(stats aliases ${SPRY_CLI_ROOT_DIRECTORY}/spryts) + $(stats aliases ${SPRY_CLI_ROOT_DIRECTORY}/user_aliases.sh) ))
local total_constants=$(( $(stats constants ${SPRY_CLI_ROOT_DIRECTORY}/spryts) + $(stats constants ${SPRY_CLI_ROOT_DIRECTORY}/user_settings.sh) ))

# Get the total number of lines
local array_of_lines=("${(@f)$(stats lines ${SPRY_CLI_ROOT_DIRECTORY}/spryts)}")  # Get the multiline string as an array
...
local total_usage=$(( ${help_usage} + ${core_usage} + ${spryt_usage} ))
...
echo -e "${M}U${X} ${B}|---------|---------|---------|---------|${X}"
echo -e "${M}S${X} ${B}|${X} ${C}$(format fill "Core" ${usage_char_limit})${X} ${B}|${X} ${C}$(format fill "Spryts" ${usage_char_limit})${X} ${B}|${X} ${C}$(format fill "Help" ${usage_char_limit})${X} ${B}|${X} ${C}$(format fill "Total" ${usage_char_limit})${X} ${B}|${X}"
echo -e "${M}A${X} ${B}|---------|---------|---------|---------|${X}"
echo -e "${M}G${X} ${B}|${X} ${C}$(format fill ${core_usage} ${usage_char_limit})${X} ${B}|${X} ${C}$(format fill ${spryt_usage} ${usage_char_limit})${X} ${B}|${X} ${C}$(format fill ${help_usage} ${usage_char_limit})${X} ${B}|${X} ${C}$(format fill ${total_usage} ${usage_char_limit})${X} ${B}|${X}"
echo -e "${M}E${X} ${B}|---------|---------|---------|---------|${X}"
echo ""
